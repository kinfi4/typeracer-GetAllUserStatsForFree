...
from src.report.data import (
    ...
    date_bounds, wpm_bounds, accuracy_bounds, available_race_sizes, available_modes,
)
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        preset = st.radio("Date range", ["All", "7d", "30d", "90d", "YTD", "1y", "Custom"], index=0)
        ...
        sizes = available_race_sizes(df_full)
        race_sizes = st.multiselect("Race size", sizes, default=sizes) if sizes else []

        modes_available = available_modes(df_full)
        selected_modes = st.multiselect("Mode", modes_available, default=modes_available) if modes_available else []
    ...
    df = filter_df(
        ...
        race_sizes=race_sizes if race_sizes and len(race_sizes) < len(sizes) else None,
        modes=selected_modes if selected_modes and len(selected_modes) < len(modes_available) else None,
    )
    ...
    global_pb = int(df_full["speed"].max())
    if "balloons_shown" not in st.session_state:
        st.session_state.balloons_shown = False
    if not st.session_state.balloons_shown and len(df) > 0 and int(df["speed"].max()) == global_pb:
        st.balloons()
...
def _tab_performance(df):
    ...
    with col2:
        section_header("Accuracy vs WPM")
